...
class RobotController:
    ...
    def createWorld(self, GUI=True, view_world=False):
        ...
        self.robot_id = p.loadURDF(urdf_file, useFixedBase=True, flags = p.URDF_USE_SELF_COLLISION)
        #self.ballId= p.loadURDF("soccerball.urdf",[0.5,0.5,1.3],useFixedBase=True)
        self.TableId =p.loadURDF("cube.urdf",[0.7,0.7,1.3],useFixedBase=True, flags=p.URDF_USE_SELF_COLLISION)
    ...
    #function to do end effector velcoity control
    ...
    def doInverseDynamics(self, th_initial, th_final, final_time=4):
        p.setRealTimeSimulation(False)
        ...
        for link_idx in range(self.num_joints+1):
            p.changeDynamics(self.robot_id, link_idx, linearDamping=0.5, angularDamping=0.5, jointDamping=1.0)
    ...
    # Feedback linear
    def feedback(self, th_initial, th_final, final_time=2, controller_gain=1600):
        p.setRealTimeSimulation(False)
        # get the desired trajectory
        q_d, dq_d, ddq_d = self.getTrajectory(th_initial, th_final, tf=final_time, dt=self.time_step)
        traj_points = q_d.shape[0]
        ...
        # for turning off link and joint damping
        for link_idx in range(self.num_joints+1):
            p.changeDynamics(self.robot_id, link_idx, linearDamping=1.0, angularDamping=1.0, jointDamping=3.0)
            p.changeDynamics(self.robot_id, link_idx, maxJointVelocity=200)
        ...
        while n < q_d.shape[0]:
            ...
            #tau += Kd * dq_d[n]       #applying joint damping here 
